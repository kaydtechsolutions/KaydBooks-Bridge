...
| M3: first real transaction adapter | Pick one verified operation in one authorized sample company; fresh master resolution, tax/account/currency validation, duplicate query, response identifiers and independent complete read-back; timeout/restart reconciliation | Durable controlled native sample posting implemented and synthetic-tested; real attempt awaits new-app permission. Earlier isolated sample write and SDK/QBWC readback passed |
...
| M7: deployment qualification | Backup/restore, least privilege identities, TLS/QBWC authentication, resource limits, security/audit review, migration drills, per-company pilot; explicitly authorized live gate only after real tests | Planned; live gate absent |

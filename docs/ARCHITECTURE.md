...
| M4: Hermes tools and document intake | Versioned narrow tools for prepare/validate/submit/status/lookup/verify/recover, immutable source bytes, field confidence and review, no raw qbXML/SQL; isolated Hermes profile contract tests | MCP/source review, revisions, spreadsheet intake, offline OCR corpus and current browser contracts qualified; complete release forms and Hermes conversation remain partial |

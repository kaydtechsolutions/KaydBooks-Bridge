"""Customer payment QBWC evidence, exact allocation effects and no-write recovery."""
# ruff: noqa: F401,F811
...
from kaydbooks_bridge.payment_evidence import resolve
...
from test_customer_payments import payment_case, response
...
from test_sample_payments import Session, queued_payment


@pytest.mark.parametrize("lost", [False, True])
@pytest.mark.parametrize("kind", ["partial", "full", "discount", "unapplied"])
def test_payment_effects_and_query_only_recovery(queued_payment, lost, kind):
    ...
    with svc._stores["company-a"].transaction() as db:
        assert attempt_count(db, "customer-payment.create") == 1
...
def test_payment_mismatched_balance_stays_unverified(queued_payment):
    ...
    simulator.rows["Invoice"][0].update(AppliedAmount="-4.00", BalanceRemaining="6.00")
...
def test_payment_browser_check_uses_owned_fresh_qbwc_evidence(queued_payment, monkeypatch):
    ...
    args = (bridge, token, "company-a", "customer-payment.create", "connector-company-a", payload)
    ...
    with store.transaction() as db:
        ...
        assert proof["balances"]["invoice-id"]["balance"] == "10.00"
...
def test_payment_missing_recovery_result_cannot_resend(queued_payment):
    ...
    assert "ReceivePaymentAddRq" in send(svc, ticket)
    call(svc, "closeConnection", ticket=ticket)
    recover(bridge, token, "company-a", job)
    ticket, _ = authenticate(svc)
    assert receive(svc, ticket, simulator.xml(send(svc, ticket))) == -1

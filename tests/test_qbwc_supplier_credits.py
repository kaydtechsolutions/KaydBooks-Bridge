"""Supplier credit QBWC evidence, exact allocation effects and no-write recovery."""
# ruff: noqa: F401,F811
...
from kaydbooks_bridge.supplier_credit_evidence import resolve
...
from test_supplier_credits import credit_case, queued_credit
...
def test_credit_effects_and_query_only_recovery(queued_credit, lost):
    ...
    with svc._stores["company-a"].transaction() as db:
        assert attempt_count(db, "supplier-credit.create") == 1
...
def test_payment_browser_check_uses_owned_fresh_qbwc_evidence(queued_credit, monkeypatch):
    ...
    args = (bridge, token, "company-a", "supplier-credit.create", "connector-company-a", payload)
    ...
    with store.transaction() as db:
        ...
        assert proof["balances"]["vendor_balance"] == "25.00"
...
def test_payment_missing_recovery_result_cannot_resend(queued_credit):
    ...
    assert "VendorCreditAddRq" in send(svc, ticket)
    call(svc, "closeConnection", ticket=ticket)
    recover(bridge, token, "company-a", job)
    ticket, _ = authenticate(svc)
    assert receive(svc, ticket, simulator.xml(send(svc, ticket))) == -1

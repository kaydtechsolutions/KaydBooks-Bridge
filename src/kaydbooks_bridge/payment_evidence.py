"""Customer-payment evidence with exact allocations, ownership and expiry."""
...
def resolve(config, policy, store, db, actor, payload, reference, now, *, txn_id=None):
    ...
    if txn_id is not None:
        from .payment_receipt import lookup_context
    ...
    if txn_id is not None:
        from .payment_receipt import append_lookup
    ...
    if txn_id is None:
        ...
        balances = receipt["invoice_balances"]
...
def resolve_qbwc(config, policy, store, db, actor, payload, reference, now):
    ...
    if (
        ...
        raise BridgeError("verified QBWC payment evidence required")

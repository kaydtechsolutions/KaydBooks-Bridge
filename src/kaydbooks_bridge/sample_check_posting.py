"""Bounded sample checks through the shared QBWC lifecycle only."""
    ...
    return digest(
        {
            ...
            "gate": policy.sample_check_posting,
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    if root.tag != "QBXML" or len(root) != 1 or root[0].tag != "QBXMLMsgsRs" or not len(root[0]):
        raise BridgeError("invalid check preflight envelope")
    collision = root[0][-1]
    if collision.tag != "CheckQueryRs" or collision.get("requestID") != run + "999":
        raise BridgeError("uncorrelated check duplicate query")
...
def gate(config, actor, policy, job, now):
    if job["operation"] != "check.create":
        raise BridgeError("check operation required")
    ...
    settings = policy.sample_check_posting
    if (
        not settings

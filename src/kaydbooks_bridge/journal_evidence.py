"""Fresh, owned QBWC journal master evidence."""
...
def resolve(config, policy, store, db, actor, payload, reference, now):
    ...
    if reference["transport"] != "qbwc":
        raise BridgeError("journals require Web Connector evidence")
    return resolve_qbwc(
        config, policy, store, db, actor, payload, reference, now, operation="journal.create"
    )
...
def require(config, policy, store, db, job, now):
    ...
    if (
        ...
        raise BridgeError("fresh owned journal evidence required")

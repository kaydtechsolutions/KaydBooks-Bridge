...
public static class ControlledSampleSupplierPayment {
 ...
 public static void CheckWrite(string xml,string expected) {
  ...
  if(batch==null||batch.ChildNodes.Count!=1||batch.FirstChild.Name!="BillPaymentCheckAddRq"||batch.FirstChild.ChildNodes.Count!=1||batch.FirstChild.FirstChild.Name!="BillPaymentCheckAdd")throw new Exception("Only one supplier payment add permitted");
  var payment=batch.FirstChild.FirstChild;
  string[] fields={"PayeeEntityRef","APAccountRef","TxnDate","BankAccountRef","RefNumber"};
  if(payment.ChildNodes.Count<6||payment.ChildNodes.Count>25)throw new Exception("Explicit payment fields and allocations required");
  for(int i=0;i<5;i++) {
...
try {
 ...
 [ControlledSampleSupplierPayment]::Run($env:KAYDBOOKS_NATIVE_DIRECTORY,$env:KAYDBOOKS_NATIVE_REQUEST_HASH,($env:KAYDBOOKS_NATIVE_READ_ONLY -eq 'true'))

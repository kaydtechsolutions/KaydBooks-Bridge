"""Explicitly gated sample-company supplier credits. Production posting is unavailable."""
    ...
    return digest(
        {
            ...
            "gate": policy.sample_supplier_credit_posting,
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    if root.tag != "QBXML" or len(root) != 1 or root[0].tag != "QBXMLMsgsRs" or not len(root[0]):
        raise BridgeError("invalid native credit preflight envelope")
    collision = root[0][-1]
    if collision.tag != "VendorCreditQueryRs" or collision.get("requestID") != run + "999":
        raise BridgeError("uncorrelated credit duplicate query")
...
def windows_exchange(request, write, folder, approve):
    ...
    return native(request, write, folder, approve, helper="native_supplier_credit.ps1")
...
def gate(config, actor, policy, job, now):
    ...
    settings = policy.sample_supplier_credit_posting
    if (
        not settings
...
def post(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            ...
            if (
                db.execute("SELECT COUNT(*) FROM native_supplier_credit_attempts").fetchone()[0]
                >= policy.sample_supplier_credit_posting["max_credits"]
            ):
            ...
            db.execute(
                "INSERT INTO native_supplier_credit_attempts VALUES (?,?,?,?,?,?,?,?)",
                (
                    job_id,
                    attempt,
                    connector.id,
                    ...
                    request,
                    context,
                    policy.sample_supplier_credit_posting["authorization"],
                ),
            )
            db.execute(
            ...
            store.event(
                ...
                actor,
                job_id,
                "supplier_credit_dispatch_prepared",
        ...
        def approve(response):
            ...
            with store.transaction() as db:
                ...
                store.event(
                    ...
                    actor,
                    job_id,
                    "supplier_credit_write_authorized"
                    if matched is None
                    else "supplier_credit_duplicate_found",
                    {
                        "preflight_hash": digest(response),
                        "supplier_credit_before": balances,
                    },
                )
            return matched is None

        folder = store.path.parent / ("native-supplier-credit-" + attempt)
        try:
            response = exchange(preflight(policy, job["payload"], run), request, folder, approve)
            if response is not None:
                matched = validate_receipt(
                    response, policy, job["payload"], run + "998", operation="VendorCreditAdd"
                )
            if matched is None:
            ...
            with store.transaction() as db:
                ...
                if current["state"] == "in-flight":
                    ...
                    store.event(
                        ...
                        actor,
                        job_id,
                        "native_supplier_credit_outcome_unknown",
                        {"attempt": attempt},
                    )
            raise
...
def reconcile(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            job = store.job(db, job_id)
            record = db.execute(
                "SELECT * FROM native_supplier_credit_attempts WHERE job_id=?", (job_id,)
            ).fetchone()
            if (
                record is None
        ...
        folder = store.path.parent / ("native-supplier-credit-reconcile-" + uuid.uuid4().hex)
    ...
    discover(
        ...
        run_id,
        supplier_credit_receipt_check={"txn_id": matched["txn_id"], "payload": job["payload"]},
        **kwargs,
    )
    from .supplier_credit_evidence import resolve
    ...
    with store.transaction() as db:
        ...
        add_response = (
            store.path.parent / ("native-supplier-credit-" + record["attempt"]) / "add.response.xml"
        )
        dispatched = None
        if add_response.exists():
            ...
            validate_receipt(
                ...
                current["payload"],
                correlation,
                operation="VendorCreditAdd",
                txn_id=matched["txn_id"],
            )
            dispatched = True
        elif db.execute(
            "SELECT 1 FROM audit WHERE job_id=? AND event='supplier_credit_duplicate_found'",
            (job_id,),
        ).fetchone():
            dispatched = False
        proof.update(origin="native-attempt-readback", bridge_dispatched=dispatched)
        baseline = db.execute(
            "SELECT data FROM audit WHERE job_id=? AND event='supplier_credit_write_authorized' ORDER BY sequence",
            (job_id,),
        ).fetchall()
        if len(baseline) != 1:
            raise BridgeError("credit requires original native supplier balance")
        proof["receipt"]["balance_effects"] = verify_balance_effect(
            current["payload"],
            json.loads(baseline[0]["data"]).get("supplier_credit_before"),
            proof["receipt"]["balances"],
        )
        db.execute(
            "UPDATE jobs SET state='verified',txn_id=?,detail='native_supplier_credit_verified' WHERE id=?",
            (matched["txn_id"], job_id),
        )
        store.event(db, bridge.clock(), actor, job_id, "native_supplier_credit_verified", proof)

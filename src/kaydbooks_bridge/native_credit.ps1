...
public static class ControlledSampleCredit {
 ...
 public static void CheckWrite(string xml,string expected) {
  ...
  if(batch==null||batch.ChildNodes.Count!=1||batch.FirstChild.Name!="CreditMemoAddRq"||batch.FirstChild.ChildNodes.Count!=1||batch.FirstChild.FirstChild.Name!="CreditMemoAdd")throw new Exception("Only one credit add permitted");
  var invoice=batch.FirstChild.FirstChild;
  foreach(XmlNode n in invoice.ChildNodes) {
   if(Array.IndexOf(new string[]{"CustomerRef","ARAccountRef","TxnDate","RefNumber","IsPending","Memo","IsToBePrinted","IsToBeEmailed","CustomerSalesTaxCodeRef","CreditMemoLineAdd"},n.Name)<0)throw new Exception("Unsupported credit field");
   if(n.Name.StartsWith("Is")&&n.InnerText!="false")throw new Exception("Unsupported credit flag");
   if(n.Name=="CreditMemoLineAdd")foreach(XmlNode field in n.ChildNodes)
    if(Array.IndexOf(new string[]{"ItemRef","Quantity","Rate","SalesTaxCodeRef"},field.Name)<0)throw new Exception("Unsupported credit line");
  }
 }
...
try {
 ...
 [ControlledSampleCredit]::Run($env:KAYDBOOKS_NATIVE_DIRECTORY,$env:KAYDBOOKS_NATIVE_REQUEST_HASH,($env:KAYDBOOKS_NATIVE_READ_ONLY -eq 'true'))

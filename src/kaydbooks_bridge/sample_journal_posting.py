"""Bounded sample journals through the shared QBWC lifecycle only."""
    ...
    return digest(
        {
            ...
            "gate": policy.sample_journal_posting,
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    if root.tag != "QBXML" or len(root) != 1 or root[0].tag != "QBXMLMsgsRs" or not len(root[0]):
        raise BridgeError("invalid journal preflight envelope")
    collision = root[0][-1]
    if collision.tag != "JournalEntryQueryRs" or collision.get("requestID") != run + "999":
        raise BridgeError("uncorrelated journal duplicate query")
...
def gate(config, actor, policy, job, now):
    if job["operation"] != "journal.create":
        raise BridgeError("journal operation required")
    ...
    settings = policy.sample_journal_posting
    if (
        not settings

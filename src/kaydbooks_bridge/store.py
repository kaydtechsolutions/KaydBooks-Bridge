...
class Store:
    def __init__(self, root: Path, company: str):
        ...
        with self.transaction() as db:
            ...
            db.execute("""CREATE TRIGGER IF NOT EXISTS supplier_payment_evidence_insert_guard
                ...
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS native_credit_attempts (
                job_id TEXT PRIMARY KEY REFERENCES jobs(id), attempt TEXT NOT NULL UNIQUE,
                connector TEXT NOT NULL, actor TEXT NOT NULL, created_at REAL NOT NULL,
                request TEXT NOT NULL, context_hash TEXT NOT NULL, authorization TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS native_credit_attempt_no_{action.lower()}
                    BEFORE {action} ON native_credit_attempts
                    BEGIN SELECT RAISE(ABORT,'native payment dispatch identity is immutable'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS native_credit_attempt_insert_guard
                BEFORE INSERT ON native_credit_attempts WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND operation='customer-credit.create' AND state='queued'
                 AND submitter=NEW.actor AND attempt IS NULL)
                BEGIN SELECT RAISE(ABORT,'native payment dispatch requires owned queued payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS check_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS check_evidence_no_{action.lower()}
                    BEFORE {action} ON check_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS check_evidence_insert_guard
            ...
            db.execute("""CREATE TABLE IF NOT EXISTS journal_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS journal_evidence_no_{action.lower()}
                    BEFORE {action} ON journal_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS journal_evidence_insert_guard
                BEFORE INSERT ON journal_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='journal.create')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            ...
            db.execute("""CREATE TABLE IF NOT EXISTS credit_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS credit_evidence_no_{action.lower()}
                    BEFORE {action} ON credit_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS credit_evidence_insert_guard
                BEFORE INSERT ON credit_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='customer-credit.create')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS native_supplier_credit_attempts (
                job_id TEXT PRIMARY KEY REFERENCES jobs(id), attempt TEXT NOT NULL UNIQUE,
                connector TEXT NOT NULL, actor TEXT NOT NULL, created_at REAL NOT NULL,
                request TEXT NOT NULL, context_hash TEXT NOT NULL, authorization TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS native_supplier_credit_attempt_no_{action.lower()}
                    BEFORE {action} ON native_supplier_credit_attempts
                    BEGIN SELECT RAISE(ABORT,'native payment dispatch identity is immutable'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS native_supplier_credit_attempt_insert_guard
                BEFORE INSERT ON native_supplier_credit_attempts WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND operation='supplier-credit.create' AND state='queued'
                 AND submitter=NEW.actor AND attempt IS NULL)
                BEGIN SELECT RAISE(ABORT,'native payment dispatch requires owned queued payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS supplier_credit_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS supplier_credit_evidence_no_{action.lower()}
                    BEFORE {action} ON supplier_credit_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS supplier_credit_evidence_insert_guard
                BEFORE INSERT ON supplier_credit_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='supplier-credit.create')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS native_refund_attempts (
                job_id TEXT PRIMARY KEY REFERENCES jobs(id), attempt TEXT NOT NULL UNIQUE,
                connector TEXT NOT NULL, actor TEXT NOT NULL, created_at REAL NOT NULL,
                request TEXT NOT NULL, context_hash TEXT NOT NULL, authorization TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS native_refund_attempt_no_{action.lower()}
                    BEFORE {action} ON native_refund_attempts
                    BEGIN SELECT RAISE(ABORT,'native payment dispatch identity is immutable'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS native_refund_attempt_insert_guard
                BEFORE INSERT ON native_refund_attempts WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND operation='customer-refund.create' AND state='queued'
                 AND submitter=NEW.actor AND attempt IS NULL)
                BEGIN SELECT RAISE(ABORT,'native payment dispatch requires owned queued payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS refund_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS refund_evidence_no_{action.lower()}
                    BEFORE {action} ON refund_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS refund_evidence_insert_guard
                BEFORE INSERT ON refund_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='customer-refund.create')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS native_application_attempts (
                job_id TEXT PRIMARY KEY REFERENCES jobs(id), attempt TEXT NOT NULL UNIQUE,
                connector TEXT NOT NULL, actor TEXT NOT NULL, created_at REAL NOT NULL,
                request TEXT NOT NULL, context_hash TEXT NOT NULL, authorization TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS native_application_attempt_no_{action.lower()}
                    BEFORE {action} ON native_application_attempts
                    BEGIN SELECT RAISE(ABORT,'native payment dispatch identity is immutable'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS native_application_attempt_insert_guard
                BEFORE INSERT ON native_application_attempts WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND operation='customer-credit.apply' AND state='queued'
                 AND submitter=NEW.actor AND attempt IS NULL)
                BEGIN SELECT RAISE(ABORT,'native payment dispatch requires owned queued payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS application_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS application_evidence_no_{action.lower()}
                    BEFORE {action} ON application_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS application_evidence_insert_guard
                BEFORE INSERT ON application_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='customer-credit.apply')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS native_supplier_application_attempts (
                job_id TEXT PRIMARY KEY REFERENCES jobs(id), attempt TEXT NOT NULL UNIQUE,
                connector TEXT NOT NULL, actor TEXT NOT NULL, created_at REAL NOT NULL,
                request TEXT NOT NULL, context_hash TEXT NOT NULL, authorization TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS native_supplier_application_attempt_no_{action.lower()}
                    BEFORE {action} ON native_supplier_application_attempts
                    BEGIN SELECT RAISE(ABORT,'native payment dispatch identity is immutable'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS native_supplier_application_attempt_insert_guard
                BEFORE INSERT ON native_supplier_application_attempts WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND operation='supplier-credit.apply' AND state='queued'
                 AND submitter=NEW.actor AND attempt IS NULL)
                BEGIN SELECT RAISE(ABORT,'native payment dispatch requires owned queued payment'); END""")
            db.execute("""CREATE TABLE IF NOT EXISTS supplier_application_evidence_links (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT, job_id TEXT NOT NULL REFERENCES jobs(id), evidence TEXT NOT NULL)""")
            for action in ("UPDATE", "DELETE"):
                db.execute(f"""CREATE TRIGGER IF NOT EXISTS supplier_application_evidence_no_{action.lower()}
                    BEFORE {action} ON supplier_application_evidence_links
                    BEGIN SELECT RAISE(ABORT,'payment evidence is append-only'); END""")
            db.execute("""CREATE TRIGGER IF NOT EXISTS supplier_application_evidence_insert_guard
                BEFORE INSERT ON supplier_application_evidence_links WHEN NOT EXISTS
                (SELECT 1 FROM jobs WHERE id=NEW.job_id AND state='draft' AND operation='supplier-credit.apply')
                BEGIN SELECT RAISE(ABORT,'payment evidence requires draft payment'); END""")
    ...
    def job(db, job_id: str) -> dict:
        ...
        evidence_table = (
            ...
            else "sales_receipt_evidence_links"
            ...
            else "refund_evidence_links"
            if result["operation"] == "customer-refund.create"
            else "application_evidence_links"
            if result["operation"] == "customer-credit.apply"
            else "credit_evidence_links"
            if result["operation"] == "customer-credit.create"
            else "supplier_payment_evidence_links"
        ...
        if receipt:
            ...
            event = (
                ...
                if result["operation"] == "supplier-credit.create"
                else "native_refund_verified"
                if result["operation"] == "customer-refund.create"
                else "native_application_verified"
                if result["operation"] == "customer-credit.apply"
                else "native_credit_verified"
                if result["operation"] == "customer-credit.create"
                else "native_supplier_payment_verified"
                if result["operation"] == "supplier-payment.create"

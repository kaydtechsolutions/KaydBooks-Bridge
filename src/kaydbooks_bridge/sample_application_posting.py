    ...
    return digest(
        {
            ...
            "gate": policy.sample_application_posting,
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    return ({"txn_id": payload["invoice_txn_id"]} if recovering else None), balances


def windows_exchange(request, write, folder, approve):
    from .sample_posting import windows_exchange as native

    return native(request, write, folder, approve, helper="native_application.ps1")
...
def gate(config, actor, policy, job, now):
    ...
    settings = policy.sample_application_posting
    if (
        not settings
...
def post(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            ...
            if (
                db.execute("SELECT COUNT(*) FROM native_application_attempts").fetchone()[0]
                >= policy.sample_application_posting["max_applications"]
            ):
            ...
            db.execute(
                "INSERT INTO native_application_attempts VALUES (?,?,?,?,?,?,?,?)",
                (
                    job_id,
                    attempt,
                    connector.id,
                    ...
                    request,
                    context,
                    policy.sample_application_posting["authorization"],
                ),
            )
            db.execute(
        ...
        def approve(response):
            ...
            with store.transaction() as db:
                ...
                store.event(
                    ...
                    actor,
                    job_id,
                    "application_write_authorized"
                    if matched is None
                    else "application_duplicate_found",
                    {
        ...
        folder = store.path.parent / ("native-application-" + attempt)
        try:
            ...
            with store.transaction() as db:
                ...
                if current["state"] == "in-flight":
                    ...
                    store.event(
                        ...
                        actor,
                        job_id,
                        "native_application_outcome_unknown",
                        {"attempt": attempt},
                    )
            raise
...
def reconcile(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            job = store.job(db, job_id)
            record = db.execute(
                "SELECT * FROM native_application_attempts WHERE job_id=?", (job_id,)
            ).fetchone()
            if (
                record is None
        ...
        folder = store.path.parent / ("native-application-reconcile-" + uuid.uuid4().hex)
    ...
    discover(
        ...
        run_id,
        application_receipt_check={"txn_id": matched["txn_id"], "payload": job["payload"]},
        **kwargs,
    )
    from .application_evidence import resolve
    ...
    with store.transaction() as db:
        ...
        add_response = (
            store.path.parent / ("native-application-" + record["attempt"]) / "add.response.xml"
        )
        dispatched = None
        if add_response.exists():
            ...
            "SELECT 1 FROM audit WHERE job_id=? AND event='application_duplicate_found'",
            (job_id,),
        ).fetchone():
            dispatched = False
        proof.update(origin="native-attempt-readback", bridge_dispatched=dispatched)
        baseline = db.execute(
            "SELECT data FROM audit WHERE job_id=? AND event='application_write_authorized' ORDER BY sequence",
            (job_id,),
        ).fetchall()
        if len(baseline) != 1:
        ...
        db.execute(
            "UPDATE jobs SET state='verified',txn_id=?,detail='native_application_verified' WHERE id=?",
            (matched["txn_id"], job_id),
        )
        store.event(db, bridge.clock(), actor, job_id, "native_application_verified", proof)

"""Customer-refund evidence with unused-credit limits, ownership and expiry."""
...
from .customer_refunds import append_check, plan, validate_check
...
def resolve(config, policy, store, db, actor, payload, reference, now, *, txn_id=None):
    ...
    if reference["transport"] != "direct-sdk":
        raise BridgeError("refund evidence currently requires direct-sdk")
    connector = config.connectors.get(identifier(reference["connector"]))
    if connector is None or connector.company != policy.id:
        raise BridgeError("refund evidence company or connector mismatch")
    ...
    if not isinstance(run, str) or not re.fullmatch(r"[1-9][0-9]{0,15}", run):
        raise BridgeError("invalid refund SDK evidence id")
    check = plan(policy, payload)
    if txn_id is not None:
        from .customer_refunds import lookup_context
    ...
    if txn_id is not None:
        from .customer_refunds import append_lookup
    ...
    if (
        ...
        raise BridgeError("verified owned exact refund master evidence required")
    times = [
        event["at"]
    ...
    if (
        ...
        raise BridgeError("refund evidence is stale; run a fresh exact check")
    if txn_id is None:
...
def require(config, policy, store, db, job, now):
    ...
    if (
        ...
        raise BridgeError("fresh owned refund evidence required")

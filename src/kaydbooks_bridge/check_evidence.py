"""Fresh, owned QBWC check master evidence."""
...
def resolve(config, policy, store, db, actor, payload, reference, now):
    ...
    if reference["transport"] != "qbwc":
        raise BridgeError("checks require Web Connector evidence")
    return resolve_qbwc(
        config, policy, store, db, actor, payload, reference, now, operation="check.create"
    )
...
def require(config, policy, store, db, job, now):
    ...
    if (
        ...
        raise BridgeError("fresh owned check evidence required")

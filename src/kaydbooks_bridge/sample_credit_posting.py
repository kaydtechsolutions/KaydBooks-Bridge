"""Explicitly gated sample-company customer credits. Production posting is unavailable."""
    ...
    return digest(
        {
            ...
            "gate": policy.sample_credit_posting,
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    collision = root[0][-1]
    if collision.tag != "CreditMemoQueryRs" or collision.get("requestID") != run + "999":
...
def windows_exchange(request, write, folder, approve):
    ...
    return native(request, write, folder, approve, helper="native_credit.ps1")
...
def gate(config, actor, policy, job, now):
    ...
    settings = policy.sample_credit_posting
    if (
        not settings
...
def post(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            ...
            if (
                db.execute("SELECT COUNT(*) FROM native_credit_attempts").fetchone()[0]
                >= policy.sample_credit_posting["max_credits"]
            ):
            ...
            db.execute(
                "INSERT INTO native_credit_attempts VALUES (?,?,?,?,?,?,?,?)",
                (
                    job_id,
                    attempt,
                    connector.id,
                    ...
                    request,
                    context,
                    policy.sample_credit_posting["authorization"],
                ),
            )
            db.execute(
        ...
        def approve(response):
            ...
            with store.transaction() as db:
                ...
                store.event(
                    ...
                    actor,
                    job_id,
                    "credit_write_authorized" if matched is None else "credit_duplicate_found",
                    {
        ...
        folder = store.path.parent / ("native-credit-" + attempt)
        try:
            response = exchange(preflight(policy, job["payload"], run), request, folder, approve)
            if response is not None:
                matched = validate_receipt(
                    response, policy, job["payload"], run + "998", operation="CreditMemoAdd"
                )
            if matched is None:
...
def reconcile(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            job = store.job(db, job_id)
            record = db.execute(
                "SELECT * FROM native_credit_attempts WHERE job_id=?", (job_id,)
            ).fetchone()
            if (
                record is None
        ...
        folder = store.path.parent / ("native-credit-reconcile-" + uuid.uuid4().hex)
    ...
    discover(
        ...
        run_id,
        credit_receipt_check={"txn_id": matched["txn_id"], "payload": job["payload"]},
        **kwargs,
    )
    from .credit_evidence import resolve
    ...
    with store.transaction() as db:
        ...
        add_response = (
            store.path.parent / ("native-credit-" + record["attempt"]) / "add.response.xml"
        )
        dispatched = None
        if add_response.exists():
            ...
            validate_receipt(
                ...
                current["payload"],
                correlation,
                operation="CreditMemoAdd",
                txn_id=matched["txn_id"],
            )
            dispatched = True
        elif db.execute(
            "SELECT 1 FROM audit WHERE job_id=? AND event='credit_duplicate_found'",
            (job_id,),
        ).fetchone():
            dispatched = False
        proof.update(origin="native-attempt-readback", bridge_dispatched=dispatched)
        baseline = db.execute(
            "SELECT data FROM audit WHERE job_id=? AND event='credit_write_authorized' ORDER BY sequence",
            (job_id,),
        ).fetchall()
        if len(baseline) != 1:
        ...
        store.event(db, bridge.clock(), actor, job_id, "native_credit_verified", proof)

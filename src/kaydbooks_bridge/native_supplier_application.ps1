...
public static class ControlledSampleSupplierApplication {
 ...
 public static void CheckWrite(string xml,string expected) {
  ...
  if(rq.Name!="BillPaymentCheckAddRq"||rq.Attributes.Count!=1||rq.Attributes["requestID"]==null||!System.Text.RegularExpressions.Regex.IsMatch(rq.Attributes["requestID"].Value,@"\A[1-9][0-9]{0,18}\z")||rq.ChildNodes.Count!=1||rq.FirstChild.Name!="BillPaymentCheckAdd")throw new Exception("Fixed application required");
  var add=rq.FirstChild;
  string vendor=Id(add.SelectSingleNode("PayeeEntityRef/ListID")),ap=Id(add.SelectSingleNode("APAccountRef/ListID")),bank=Id(add.SelectSingleNode("BankAccountRef/ListID"));
 ...
 public static void Run(string root,string hash,bool readOnly) {
  ...
  try {
   ...
   foreach(XmlNode q in batch.ChildNodes)
    if(Array.IndexOf(new string[]{"HostQueryRq","CompanyQueryRq","PreferencesQueryRq","AccountQueryRq","VendorQueryRq","BillQueryRq","VendorCreditQueryRq","BillToPayQueryRq"},q.Name)<0)throw new Exception("Unsupported preflight request");
...
try {
 ...
 [ControlledSampleSupplierApplication]::Run($env:KAYDBOOKS_NATIVE_DIRECTORY,$env:KAYDBOOKS_NATIVE_REQUEST_HASH,($env:KAYDBOOKS_NATIVE_READ_ONLY -eq 'true'))

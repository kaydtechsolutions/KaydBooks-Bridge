"""Customer-credit evidence with original-invoice limits, ownership and expiry."""
...
from .customer_credits import append_check, plan, validate_check
...
def resolve(config, policy, store, db, actor, payload, reference, now, *, txn_id=None):
    ...
    if reference["transport"] == "qbwc":
        ...
        if txn_id is not None:
            raise BridgeError("standalone QBWC credit receipt checks are unavailable")
        return resolve_qbwc(
            config,
            policy,
            store,
    ...
    if txn_id is not None:
        from .customer_credits import lookup_context
    ...
    if txn_id is not None:
        from .customer_credits import append_lookup

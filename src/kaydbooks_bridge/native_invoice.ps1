 ...
 public static void Run(string root,string hash,bool readOnly) {
  ...
  try {
   ...
   foreach(XmlNode q in batch.ChildNodes)
    if(Array.IndexOf(new string[]{"HostQueryRq","CompanyQueryRq","PreferencesQueryRq","AccountQueryRq","CustomerQueryRq","ItemServiceQueryRq","ItemInventoryQueryRq","ItemDiscountQueryRq","ItemOtherChargeQueryRq","SalesTaxCodeQueryRq","PriceLevelQueryRq","InvoiceQueryRq"},q.Name)<0)throw new Exception("Unsupported preflight request");

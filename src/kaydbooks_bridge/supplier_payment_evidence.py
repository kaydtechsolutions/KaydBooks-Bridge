"""Supplier-payment evidence with exact allocations, ownership and expiry."""
...
def resolve(config, policy, store, db, actor, payload, reference, now, *, txn_id=None):
    ...
    if not isinstance(run, str) or not re.fullmatch(r"[1-9][0-9]{0,15}", run):
        raise BridgeError("invalid payment SDK evidence id")
    check = plan(policy, payload)
    if txn_id is not None:
        from .supplier_payment_receipt import lookup_context
    ...
    if txn_id is not None:
        from .supplier_payment_receipt import append_lookup
    ...
    if txn_id is None:
        ...
        balances = receipt["bill_balances"]

"""Explicitly gated sample-company supplier payments. Production posting is unavailable."""
...
from .supplier_payment_receipt import (
    add_request,
    append_query,
    validate_receipt,
    verify_balance_effect,
)
from .supplier_payments import append_check, plan, validate_check
...
def check_preflight(response, policy, payload, connector, run, *, recovering=False):
    ...
    collision = root[0][-1]
    if collision.tag != "BillPaymentCheckQueryRs" or collision.get("requestID") != run + "999":
...
def windows_exchange(request, write, folder, approve):
    ...
    return native(request, write, folder, approve, helper="native_supplier_payment.ps1")
...
def gate(config, actor, policy, job, now):
    ...
    settings = policy.sample_supplier_payment_posting
    if (
        not settings
...
def post(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            ...
            if (
                db.execute("SELECT COUNT(*) FROM native_supplier_payment_attempts").fetchone()[0]
                >= policy.sample_supplier_payment_posting["max_payments"]
            ):
            ...
            db.execute(
                "INSERT INTO native_supplier_payment_attempts VALUES (?,?,?,?,?,?,?,?)",
                (
                    job_id,
                    attempt,
                    connector.id,
                    ...
                    request,
                    context,
                    policy.sample_supplier_payment_posting["authorization"],
                ),
            )
            db.execute(
            ...
            store.event(
                ...
                actor,
                job_id,
                "supplier_payment_dispatch_prepared",
        ...
        def approve(response):
            ...
            with store.transaction() as db:
                ...
                store.event(
                    ...
                    actor,
                    job_id,
                    "supplier_payment_write_authorized"
                    if matched is None
                    else "supplier_payment_duplicate_found",
                    {
                        "preflight_hash": digest(response),
                        "bill_before": balances,
                    },
                )
            return matched is None

        folder = store.path.parent / ("native-supplier-payment-" + attempt)
        try:
            ...
            with store.transaction() as db:
                ...
                if current["state"] == "in-flight":
                    ...
                    store.event(
                        ...
                        actor,
                        job_id,
                        "native_supplier_payment_outcome_unknown",
                        {"attempt": attempt},
                    )
            raise
...
def reconcile(bridge, token, company, job_id, *, exchange=windows_exchange, read_exchange=None):
    ...
    with company_lock(store.path.with_suffix(".sdk.lock")):
        with store.transaction() as db:
            job = store.job(db, job_id)
            record = db.execute(
                "SELECT * FROM native_supplier_payment_attempts WHERE job_id=?", (job_id,)
            ).fetchone()
            if (
                record is None
    ...
    discover(
        ...
        run_id,
        supplier_payment_receipt_check={"txn_id": matched["txn_id"], "payload": job["payload"]},
        **kwargs,
    )
    from .supplier_payment_evidence import resolve
    ...
    with store.transaction() as db:
        ...
        add_response = (
            store.path.parent
            / ("native-supplier-payment-" + record["attempt"])
            / "add.response.xml"
        )
        dispatched = None
        if add_response.exists():
            ...
            "SELECT 1 FROM audit WHERE job_id=? AND event='supplier_payment_duplicate_found'",
            (job_id,),
        ).fetchone():
            dispatched = False
        proof.update(origin="native-attempt-readback", bridge_dispatched=dispatched)
        baseline = db.execute(
            "SELECT data FROM audit WHERE job_id=? AND event='supplier_payment_write_authorized' ORDER BY sequence",
            (job_id,),
        ).fetchall()
        if len(baseline) != 1:
            raise BridgeError("payment requires original native bill balances")
        proof["receipt"]["balance_effects"] = verify_balance_effect(
            current["payload"],
            json.loads(baseline[0]["data"]).get("bill_before"),
            proof["receipt"]["bill_balances"],
        )
        db.execute(
            "UPDATE jobs SET state='verified',txn_id=?,detail='native_supplier_payment_verified' WHERE id=?",
            (matched["txn_id"], job_id),
        )
        store.event(db, bridge.clock(), actor, job_id, "native_supplier_payment_verified", proof)

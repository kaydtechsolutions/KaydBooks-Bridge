"""Fresh, owned QBWC inventory_transfer master evidence."""
...
def resolve(config, policy, store, db, actor, payload, reference, now):
    ...
    if reference["transport"] != "qbwc":
        raise BridgeError("inventory_transfers require Web Connector evidence")
    return resolve_qbwc(
        config,
        policy,
        store,
...
def require(config, policy, store, db, job, now):
    ...
    if (
        ...
        raise BridgeError("fresh owned inventory_transfer evidence required")

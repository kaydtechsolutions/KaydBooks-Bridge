"""Supplier-credit evidence with exact bill/credit links, ownership and expiry."""
...
from .supplier_application import append_check, plan, validate_check
...
def resolve(config, policy, store, db, actor, payload, reference, now, *, txn_id=None):
    ...
    if txn_id is not None:
        from .supplier_application import lookup_context
    ...
    if txn_id is not None:
        from .supplier_application import append_lookup

...
> Archived upstream documentation, retained for transport reference and attribution.
> These examples are not the KaydBooks Bridge application and do not enforce its
> company permissions or durable queue. Upstream capability/status-code claims need
> independent SDK verification. Start with [the Bridge README](README.md).
